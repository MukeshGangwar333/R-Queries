...
Query <- paste("SELECT blstudy_atul.ETO_OFR_DISPLAY_ID, FK_GLCAT_MCAT_ID, ETO_OFR_TITLE, GLUSR_USR_CITY, GLUSR_USR_STATE, FK_IM_SPEC_MASTER_DESC, FK_IM_SPEC_OPTIONS_DESC, 
               UNIQUE_SOLD, TOTAL_SOLD, RETAIL_FLAG, EMAIL_FLAG,PRIME_MCAT_NAME,FREQUENCY,LOOKING_FOR_SUPPLIERS,PORPOSE, 
               LISTAGG(SECONDARY_MCAT, '|')
               WITHIN GROUP (ORDER BY SECONDARY_MCAT) SECONDARY_MCAT
               FROM  blstudy_atul,blstudy_atul2 
               where blstudy_atul.ETO_OFR_DISPLAY_ID = blstudy_atul2.ETO_OFR_DISPLAY_ID
               GROUP BY blstudy_atul.ETO_OFR_DISPLAY_ID, FK_GLCAT_MCAT_ID, ETO_OFR_TITLE, GLUSR_USR_CITY, 
               GLUSR_USR_STATE, FK_IM_SPEC_MASTER_DESC, FK_IM_SPEC_OPTIONS_DESC, 
               UNIQUE_SOLD, TOTAL_SOLD, RETAIL_FLAG, EMAIL_FLAG,PRIME_MCAT_NAME,FREQUENCY,LOOKING_FOR_SUPPLIERS,PORPOSE")
