...
Data2 <- read_excel("C:/Users/imart/Desktop/Excavator/Buy lead data/Excavator_label_final.xlsx", sheet = 1)


##Removing products which are wrongly mapped to other subcats
Data2 <-  Data2[Data2$PMCAT2!="Agricultural Equipment" &  Data2$PMCAT2!="Bulk Handling Equipment"  & Data2$PMCAT2!="Transport Tanks",]
...
#PMCAT_count1 <- PMCAT_count[c(1:4,6:10,12:13,15), ]
#PMCAT_count1 <- PMCAT_count1$Var1
#Data2 <- Data1[Data2$PARENT_NAME %in% PMCAT_freq1, ]
...
MCAT_count <- data.frame(table(Data2$GLCAT_MCAT_NAME ))
...
Freq_of_Freq <- data.frame(table(MCAT_count$Number_of_Products ))
...
###computing multiplier by dividing the greatest number with the number


str(MCAT_count)

MCAT_count$MCAT_NAME <- as.character(MCAT_count$MCAT_NAME)
...
labelled_data <- Data2[ Data2$GLCAT_MCAT_NAME %in% mcatzer0, ]
...
for (k in 1 : nrow(MCAT_count)) {
  ...
  ind <- Data2[Data2$GLCAT_MCAT_NAME ==MCAT_count$MCAT_NAME[k], ]
...
New_MCat_Count <- data.frame(table(labelled_data$GLCAT_MCAT_NAME ))
...
MCAT_Training_PATH <- paste0("C:/Users/imart/Documents/Excavator/MCAT/")
PMCAT_Testing_PATH <- paste0("C:/Users/imart/Documents/Excavator/PMCAT/")
Kfold_PATH <- paste0("C:/Users/imart/Documents/Excavator/K Fold/")
...
df1 <- df[,c("FK_GLCAT_MCAT_ID","GLCAT_MCAT_NAME","PMCAT2","FK_PARENT_MCAT_ID","PMCAT_NAME",  "ETO_OFR_DISPLAY_ID","ETO_OFR_TITLE", "FK_IM_SPEC_MASTER_DESC","Label")]
#df1 <- df[,c("PRIME_MCAT_ID","PMCAT2","PRIME_MCAT_NAME","PC_ITEM_ID","PC_ITEM_NAME","PC_ITEM_DESC_SMALL","Label1")]
#df1 <- df[,c(1:5,8,9)]
#fix(df)
#df2 <- df1[1:2000,]

df2 <- df1
colnames(df2)[colnames(df2)=="FK_IM_SPEC_MASTER_DESC"] <- "New_Desc"
table(df2$GLCAT_MCAT_NAME )
table(df2$PMCAT2 )
#?cut
...
df2$New_Desc_final <- paste(df2$New_Desc,  df2$New_Desc1,df2$New_Desc2,df2$New_Desc3,df2$New_Desc4,df2$New_Desc5, df2$New_Desc6)
...
df2$New_Desc_final <- gsub("</b>","",df2$New_Desc_final)
df2$New_Desc_final <- trimws(df2$New_Desc_final)
...
length(!duplicated(df_check2$ETO_OFR_DISPLAY_ID ))
...
data_unique$ETO_OFR_TITLE1 <- tolower(data_unique$ETO_OFR_TITLE)
data_unique$ETO_OFR_TITLE1 <- rm_white(data_unique$ETO_OFR_TITLE1)
data_unique$ETO_OFR_TITLE1 <- gsub("[[:punct:]]","",data_unique$ETO_OFR_TITLE1)
#data_unique$ETO_OFR_TITLE1 <- gsub("[0-9] \\w+ *", "",data_unique$ETO_OFR_TITLE1)
#data_unique$ETO_OFR_TITLE1 <- gsub("[0-9]\\w+ *", "",data_unique$ETO_OFR_TITLE1)
#data_unique$ETO_OFR_TITLE1 <- gsub("[[:digit:]]","",data_unique$ETO_OFR_TITLE1)
#data_unique$ETO_OFR_TITLE1 <- rm_white(data_unique$ETO_OFR_TITLE1)
#data_unique$Label2 <- NULL

colnames(data_unique)[colnames(data_unique)=="Label" ] <- "Label1"
...
data_unique$Label1 <- gsub("[0-9]","",data_unique$Label1)
#data_unique$Label1 <- tolower(data_unique$Label1)
data_unique$concat_name <- paste(data_unique$ETO_OFR_TITLE ,data_unique$Label1,data_unique$New_Desc)
...
###data_unique <- data_unique[data_unique$PMCAT2!="", ]

data_unique$PRIME_PMCAT_NAME1 <- gsub(" ","_",data_unique$PMCAT2)
data_unique$PRIME_MCAT_NAME1 <- gsub(" ","_",data_unique$GLCAT_MCAT_NAME )
...
data_unique$ETO_OFR_TITLE <- gsub(","," ",data_unique$ETO_OFR_TITLE)
data_unique$ETO_OFR_TITLE <- rm_white(data_unique$ETO_OFR_TITLE)
...
Final_data$ETO_OFR_TITLE <- gsub(","," ",Final_data$ETO_OFR_TITLE)
