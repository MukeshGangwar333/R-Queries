...
df2 <- labelled_data
#######################################
df2$united <- paste(df2$PRIME_MCAT_ID, df2$PMCAT_ID , sep= "_")
  
df <- df2[!duplicated(df2$PRIME_MCAT_ID ), ]
  
  
  write.csv(df,"df2_pmcat_check.csv", row.names = F )
...
data_unique$PRIME_MCAT_NAME <- rm_white(data_unique$PRIME_MCAT_NAME)
data_unique$PRIME_MCAT_NAME1 <- gsub(" ","_",data_unique$PRIME_MCAT_NAME)
data_unique$PRIME_MCAT_NAME1 <- paste0(data_unique$PRIME_MCAT_NAME1,"_",data_unique$PMCAT_ID)
...
data_unique$PMCAT_C[Blank_PMCAT_INDEX] <- paste0(data_unique$PMCAT_C[Blank_PMCAT_INDEX],"_",data_unique$PMCAT_ID[Blank_PMCAT_INDEX])
...
data_unique$OTHER_MCATS1[Other_blank_index] <- paste0("__label__s_",data_unique$OTHER_MCATS1[Other_blank_index],"_",data_unique$PMCAT_ID[Other_blank_index])

Blank_PMCAT_INDEX <- which(data_unique$PMCAT2!= "")

sum(data_unique$PMCAT_ID=="")
...
data_unique2$PRIME_MCAT_NAME1 <- gsub("_$","",data_unique2$PRIME_MCAT_NAME1)
...
write.table(data_unique2$FASTTEXT_LABEL,"~/5 Subcats/nps_model_5subcat.txt",row.names = F,col.names = F,quote = F)
