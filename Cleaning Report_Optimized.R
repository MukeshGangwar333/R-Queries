...
driver <- rsDriver(remoteServerAddr = "localhost",browser=c("chrome"), chromever = "74.0.3729.6",extraCapabilities = eCaps)   #starting selenium server for chrome browser
remDr <- driver$client
...
mcats$IDs <- as.character(mcats$IDs)
mcats$MCAT <- trimws(mcats$MCAT)
mcats$IDs <- trimws(mcats$IDs)
...
for (j in 1:nrow(mcats)) {
  ...
  mcat_name_search_box$clickElement()
  mcat_name_search_box$clearElement()
  mcat_name_search_box$clickElement()
  mcat_name_search_box$sendKeysToActiveElement(list(mcats$MCAT[j]))
  ...
  #ser3 <- grep(paste0("^",mcats$MCAT[j],"\\b") ,ser2,ignore.case = T )
  ser3 <- grep(paste0(" ",mcats$IDs[j]," ") ,ser2,ignore.case = T )
  ...
  #rm(mcat_name_search_box,ser,ser1,ser2,ser3,search_result,button,report_data,report_data1,report_data3,report_data4)
  
  }  else if (report_data4[2] != 0) {
...
rm(mcat_name_search_box,ser,ser1,ser2,ser3,search_result,button,report_data,report_data1,report_data3,report_data4,report_data5,issue_type,files,dirs,lastfiles,issue_type1,Rejected,rr,datee,datee1,datee2,my_data,last_files)	
last_files <- ls()
